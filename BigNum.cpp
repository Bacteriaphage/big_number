...
bool BigNum::swap(vector<unsigned int> &a, vector<unsigned int> &b) {
    ...
    for (int i = 0; i < a.size()-1;) {                       //make sure there are no zero in head
    ...
    if (a.size() < b.size()) {                               //b is greater than a then swap
        temp = a;
        a = b;
        b = temp;
        return true;
    ...
    if (a.size() == b.size()) {
        for (int i = 0; i < a.size(); i++) {
            if (a[i] > b[i]) break;
            if (a[i] < b[i])                                 //b is greater than a then swap
            {
                temp = a;
                a = b;
                b = temp;
...
string BigNum::add(vector<unsigned int> &a, vector<unsigned int> &b) {
    ...
    vector<int> result;
    string output;
    for (int i = 0; i < a.size(); i++) {                      //use a to loop
        if (i < b.size()) {
            thisDigit = a[a.size() - 1 - i] + b[b.size() - 1 - i] + carry;
            result.push_back(thisDigit % 10);                 //calculate digit
            carry = thisDigit / 10;                           //calculate carry
        }
        else {
    ...
    for (int i = 0; i < result.size(); i++) {                 //transfer result to string
...
string BigNum::minus(vector<unsigned int> &a, vector<unsigned int> &b) {
    ...
    for (int i = 0; i < a.size();i++) {
        if (i < b.size()) {
            thisDigit = a[a.size() - 1 - i] - b[b.size() - 1 - i] - carry;
            result.push_back((thisDigit + 10) % 10);           //calculate digit
            carry = thisDigit < 0 ? 1 : 0;                     //calculate carry
        }
        else {
            thisDigit = a[a.size() - 1 - i] - carry;
...
bool operator> (string a, string b) {
    string result = BigNum::operationManager(a, "-", b);
...
bool operator> (string a, int b) {
    string result = BigNum::operationManager(a, "-", std::to_string(b));
    if (result[0] != '-' && result != "0") {
        return true;
    }
    else return false;
...
string operator+(string a, string b) {
    return BigNum::operationManager(a, "+", b);
}
string operator-(string a, string b) {
    return BigNum::operationManager(a, "-", b);
...
string operator%(string a,string b) {
    return BigNum::operationManager(a, "mod", b);
}


string operator+(string a, int b) {
    return BigNum::operationManager(a, "+", std::to_string(b));
}
...
string operator%(string a, int b) {
    return BigNum::operationManager(a, "mod", std::to_string(b));
...
string BigNum::powerMod(string a, string n, string m) {
    string prod = "1";
    while (n > 0) {
        if (n % 2 == 1) {
            prod = (prod * a) % m;
